 ...
 *
 * `CARD_FEE_RATE` = 씨마켓 결제대행 카드 요율 **1.87%**(대표 확인 2026-09-17). 카드수수료 = (물품금액 + 심부름값) × 요율,
 * 원 단위 올림. 결제대행사는 수수료를 얹은 결제 금액 전체에 요율을 매기므로, 얹은 수수료분의 요율(금액의 약 0.035%)은
 * 씨마켓이 부담한다. null 로 두면 화면은 숫자 없이 «카드수수료 별도(결제 시 확정)» 로 적는다.
 */

export const ERRAND_FEE_PER_REQUEST = 2_000

export const CARD_FEE_RATE: number | null = 0.0187
