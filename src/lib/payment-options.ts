 ...
 */
export async function getPaymentOptions(memberKey: string): Promise<PaymentOptions> {
  if (isSemoConfigured()) return semoGetPaymentOptions(memberKey)
  return stubPaymentOptions(memberKey)
}
